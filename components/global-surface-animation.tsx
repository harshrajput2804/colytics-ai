...
export function GlobalSurfaceAnimation() {
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      // Keep a visible baseline even under light UI layers.
      const base = 0.075
      ...
      const beamAlpha = 0.03 + intensity * 0.07
      ...
      for (let layer = 0; layer < 3; layer++) {
        ...
        context.fillStyle = `rgba(21,73,240,${0.03 + intensity * 0.03
          })`
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        const x =
          baseX +
          Math.sin(node.seed + t * 0.02) * 80

        const y =
          height * 0.5 +
          Math.cos(node.seed * 1.3 + t * 0.03) * 250
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        context.fillStyle = `rgba(21,73,240,${0.25 + intensity * 0.4
          })`
      ...
      for (let i = 0; i < 12; i++) {
        ...
        context.fillStyle = `rgba(255,255,255,${0.35 + intensity * 0.4
          })`
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        context.fillStyle = `rgba(21,73,240,${particleAlpha *
          (0.6 + (i % 5) * 0.08)
          })`
    ...
    resize()
    updateScrollState()
    // Always draw at least one frame so the surface is visible even when
    // the OS/browser has reduced motion enabled.
    rafId = window.requestAnimationFrame(draw)
