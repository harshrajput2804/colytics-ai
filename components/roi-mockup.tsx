              ...
              <table className="w-full min-w-[760px] border-collapse text-sm">
                <thead>
                  <tr className="border-b border-gray-300 text-left text-[10px] uppercase tracking-[0.25em] text-gray-600">
                    <th className="w-[38%] pb-3 pr-4 font-medium">Module & Lifecycle Work</th>
                    <th className="whitespace-nowrap pb-3 pr-4 font-medium">Manual</th>
                    <th className="whitespace-nowrap pb-3 pr-4 font-medium">Multiple Tools</th>
                    <th className="whitespace-nowrap pb-3 pr-4 font-medium">Colytics AI</th>
                    <th className="whitespace-nowrap pb-3 pr-4 text-right font-medium">Save vs Manual</th>
                    <th className="whitespace-nowrap pb-3 text-right font-medium">Save vs Tools</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((row) => {
                    ...
                    return (
                      <tr key={row[0]} className="border-b border-gray-200 align-top">
                        ...
                        <td className="whitespace-nowrap py-2.5 pr-4 text-gray-700 tabular-nums">{row[1].toFixed(1)}</td>
                        <td className="whitespace-nowrap py-2.5 pr-4 text-gray-700 tabular-nums">{row[2].toFixed(1)}</td>
                        <td className="whitespace-nowrap py-2.5 pr-4 text-gray-700 tabular-nums">{row[3].toFixed(2)}</td>
                        <td className="whitespace-nowrap py-2.5 pr-4 text-right font-medium text-gray-900 tabular-nums">{saveManual.toFixed(saveManual >= 10 ? 1 : 2)} h</td>
                        <td className="whitespace-nowrap py-2.5 text-right font-medium text-gray-900 tabular-nums">{saveTools.toFixed(saveTools >= 10 ? 1 : 2)} h</td>
                      </tr>
                    )
                  })}
                  <tr className="border-b border-gray-300 bg-gray-50 font-semibold">
                    <td className="py-3 pr-4">Total per site / month</td>
                    <td className="whitespace-nowrap py-3 pr-4 tabular-nums">68.0</td>
                    <td className="whitespace-nowrap py-3 pr-4 tabular-nums">28.5</td>
                    <td className="whitespace-nowrap py-3 pr-4 tabular-nums">5.25</td>
                    <td className="whitespace-nowrap py-3 pr-4 text-right tabular-nums">62.75 hrs</td>
                    <td className="whitespace-nowrap py-3 text-right tabular-nums">23.25 hrs</td>
                  </tr>
                </tbody>
              </table>
            </div>
